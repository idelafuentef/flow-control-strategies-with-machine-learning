...
parameters.size=150;                                         % number of parameters of the pool1
parameters.sensors=22;                                       % number of sensors(Ns)
...
parameters.opsetrange=[1,2,3,5,6,9];            %operations(see MLC_tools/opset.m)
% 1  addition       (+)
% 2  substraction   (-)
% 3  multiplication (*)
% 4  division       (%)
...
parameters.maxdepth=5;                                 %max depth of a tree
...
parameters.tournamentsize=round(parameters.size/100);          %number of individuals selected for the tournament(Np)
