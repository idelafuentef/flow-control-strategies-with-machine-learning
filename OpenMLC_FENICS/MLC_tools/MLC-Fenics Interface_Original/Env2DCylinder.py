...
# ------------------------------------------------------------------------
    ...
    #MODIFIED FROM ORIGINAL RABAULT CODE (Ignacio de la Fuente, 15/05/21)
    #executes the fenics solver, updating the actions (jet actuations) for a certain number of execution steps (currently 50, same as in original rabault code)
    ...
    def execute(self,fun_jet1,fun_jet2,dt):
        ...
        Cd=self.drag+0.159
        Cd_167=self.history_parameters["drag"].get()[-168]+0.159
        Cd_333=self.history_parameters["drag"].get()[-334]+0.159
        Cd_500=self.history_parameters["drag"].get()[-501]+0.159
        der_Cd=(self.history_parameters["drag"].get()[-1]-self.history_parameters["drag"].get()[-2])/dt
        
        Cl=self.lift
        Cl_167=self.history_parameters["lift"].get()[-168]
        Cl_333=self.history_parameters["lift"].get()[-334]
        Cl_500=self.history_parameters["lift"].get()[-501]
        der_Cl=(self.history_parameters["lift"].get()[-1]-self.history_parameters["lift"].get()[-2])/dt
        t=self.solver_step*dt
