...
% coefficients of the individuals in the basket
...
%% Parameters
	basket_labels = gMLC_ind.basket_labels;
    BS = size(basket_labels,1);
    OutputNumber = size(gMLC_table.individuals(basket_labels(1)).control_law,1);
...
%% Initialization
   % Build M0 matrix (1: vertices, 2 coefficients
        ...
        M0c = cell(BS,2);
        for p=1:BS
           M0c{p,1} = gMLC_table.individuals(basket_labels(p)).vertices;
           M0c{p,2} = gMLC_table.individuals(basket_labels(p)).coefficients;
           AllV = [AllV;M0c{p,1}];
        end
