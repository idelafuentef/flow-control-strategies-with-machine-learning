...
%% Labels
    ID1 = gMLC.basket.labels(1);
    IDend = gMLC.basket.labels(end);
    IDs = gMLC.basket.labels(2:end);
    if isempty(gMLC.basket.to_build)
        return
    end
    labels = gMLC.basket.to_build(:,1);
    
%% Test
    for p=1:size(gMLC.basket.to_build,1)
...
%% Update propertiesi
    gMLC.basket.status.last_operation = 'Subtitute computed';
