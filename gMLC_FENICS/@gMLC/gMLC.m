...
properties
    ...
    % Sets
        basket % simplex set
        stock % Latin Hypercube Sampling (LHS) set
        landscapebasket
    % Data base
        table
    % Plot
...
methods
    ...
    % Steps
        ...
        obj = step_exploration(obj); % LHS step
        obj = step_evolution(obj); % LHS step
    ...
    % Plot control law
     ...
     % Proximity map
        % prior
        obj=compute_distance_prior(obj);
        proximity_map_prior(obj,cycle,save);
        project_VIP(obj);
        export_proxi_map_prior(obj);
        % true
        extract_to_compute_distance(obj);
        obj = compute_distance(obj);
        proximity_map(obj,cycle,save);
        export_proxi_map(obj);
    % Give information concerning individuals
        give(obj,IDs);
        best_individuals(obj,Iend);
    % Add VIP
        obj=add_VIP(obj,control_law,Name);
    % Interpolation
    ...
    % Landscape
        PopulationLabels = build_evolution_basket(obj);
...
methods
    ...
    function obj = gMLC(new_parameters)
        ...
        % Properties initialization
            obj.basket = gMLCbasket(obj.parameters);
            obj.stock = gMLCstock(obj.parameters.stock_size);
            obj.landscapebasket = [];
    ...
    function val = get(obj,prop)
      ...
      if (nargin ==1)
        ...
        switch (prop)
          ...
          case 'stock'
            val = obj.stock;
          case 'landscapebasket'
            val = obj.landscapebasket;
          case 'table'
            val = obj.table;
          case 'history'
            val = obj.history;
