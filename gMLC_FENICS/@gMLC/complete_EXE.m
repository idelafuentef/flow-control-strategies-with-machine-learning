...
%% COMPLETE - Fill the waiting control laws  (Import values)
    % Which control laws
    waiting_labels = gMLC.basket.waiting_room.labels;
    if nargin<3
    ...
    % Fill the waiting list
        gMLC.basket.waiting_room.costs(:) = gMLC.table.costs(waiting_labels);
...
%% Finish optimization types
    ...
    switch optimization_type
        case 'Monte Carlo'
            ...
            gMLC.basket.status.cycle=gMLC.basket.status.cycle+1;
            gMLC.history.cycle(1)=gMLC.history.cycle(1)+1;
            
            % evaluation order
            basketcycle = gMLC.basket.status.cycle;
            for p=1:numel(waiting_labels)
                gMLC.table.individuals(waiting_labels(p)).evaluation_order = [basketcycle,waiting_labels(p)];
            end
        ...
        case 'exploitation'
            ...
            basketcycle = gMLC.basket.status.cycle+1;
            gMLC.table.individuals(lr).evaluation_order = [basketcycle,1];
            ...
            if Jr >= Jend_minus_one
                if Jc >= Jend
                    ...
                    fact_type = 14;
                    % evaluation order
                    gMLC.table.individuals(lc).evaluation_order = [basketcycle,2];
                    for nl=1:numel(ls)
                        gMLC.table.individuals(ls(nl)).evaluation_order = [basketcycle,2+nl];
                    end
            ...
            elseif Jr < J1
                ...
                gMLC.table.individuals(le).evaluation_order = [basketcycle,2];
...
%             Jexp = waiting_costs((3+BS):end); lexp = waiting_labels((3+BS):end);
            %             % exploration
            %             if not(strcmp(exploration,'none'))
            %                 new_labels = [labels;lexp];
            %                 new_costs = [costs;Jexp];
            %                 [~,idx] = sort(new_costs);
        ...
        case 'evolution'
            ...
            basketcycle = gMLC.basket.status.cycle+1;
            for p=1:numel(waiting_labels)
                gMLC.table.individuals(waiting_labels(p)).evaluation_order = [basketcycle+0.5,p];
            end
...
%% Update basket (?what about gMLC_basket.update?)
      % in the basket
      gMLC.basket.labels = new_labels;
      gMLC.basket.costs = new_costs;
      % erase waiting room
      gMLC.basket.waiting_room.labels = []; % structure : labels, costs
      gMLC.basket.waiting_room.costs = []; % structure : labels, costs
      % Stat

%% Update properties
      gMLC.basket.status.evaluated = 'evaluated';
      gMLC.basket.status.last_operation = 'complete';
      gMLC.basket.status.individuals_to_evaluate = [];
