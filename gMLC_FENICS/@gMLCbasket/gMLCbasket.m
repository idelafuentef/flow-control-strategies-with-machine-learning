classdef gMLCbasket < handle
% gMLCbasket class definition file
%
...
%% External methods
methods
    % Generate the first basket
        obj = generate_random(obj,gMLC_table,gMLC_parameters);
    % Main methods
        % exploitation
        obj = downhill_simplex(obj,gMLC_table,gMLC_parameters);
        % exploration
        obj = add_new_random(obj,stock,gMLC_parameters);
        obj = add_new_LHS(obj,stock,gMLC_table,gMLC_parameters);
    % Downhill steps
...
methods
    % Constructor
    function obj = gMLCbasket(gMLC_parameters)
%         Properties initialization
            obj.initial_individuals.labels = zeros(gMLC_parameters.basket_init_size,1);
            obj.initial_individuals.evaluated = false(gMLC_parameters.basket_init_size,1);
            obj.labels = zeros(gMLC_parameters.basket_size,1);
            obj.costs = -1*ones(gMLC_parameters.basket_size,1);
            ...
            obj.dist_list = -1*ones(gMLC_parameters.basket_size,1);
