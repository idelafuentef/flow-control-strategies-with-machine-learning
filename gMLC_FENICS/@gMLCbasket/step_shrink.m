function gMLC_basket=step_shrink(gMLC_basket,gMLC_table,gMLC_parameters)
% gMLCbasket class step_shrink method
%
...
%% Buil the control laws
    ...
    for p=1:length(IDs)
		...
		if WeightedMatrix
            Indm.basket_labels = gMLC_basket.labels;
...
% External evaluation test
    if strcmp(problem_type,'external')
      ...
      if VERBOSE > 3, fprintf('Ready.\n'),end
        % Nothing to continue
        % Update
        gMLC_basket.status.last_operation = 'shrink';
      return
    end
...
% But if there were to not be evaluated, they need to be added to the array
    ...
    for p=1:length(IDs)
      ...
      N_ev = length(gMLC_basket.status.individuals_to_evaluate)+p-length(IDs);
...
%% Update properties
    % Individuals informations
        gMLC_basket.waiting_room.labels = [gMLC_basket.waiting_room.labels;ID];
        gMLC_basket.waiting_room.costs = [gMLC_basket.waiting_room.costs;Jm];
    % Stat
        gMLC_basket.status.last_operation = 'shrink';
