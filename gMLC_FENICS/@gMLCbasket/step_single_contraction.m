function gMLC_basket=step_single_contraction(gMLC_basket,gMLC_table,gMLC_parameters)
% gMLCbasket class step_single_contraction method
%
...
%% Compute single contraction
    ...
    Jend = gMLC_basket.costs(end);
        ...
        if WeightedMatrix
                Indc = gMLCind;
                Indc.basket_labels = gMLC_basket.labels;
...
% External evaluation test
    if strcmp(problem_type,'external')
      ...
      if VERBOSE > 3, fprintf('Ready.\n'),end
        ...
        % Update
        gMLC_basket.status.last_operation = 'single contraction';
      return
    end
...
%% Update properties
    % Individuals informations
        gMLC_basket.waiting_room.labels = [gMLC_basket.waiting_room.labels;ID];
        gMLC_basket.waiting_room.costs = [gMLC_basket.waiting_room.costs;Jc];
    % Stat
        gMLC_basket.status.last_operation = 'single contraction';
