% function [x,y] = convergence(MLC,plt)
...
%           if HowToPlot==1
%               J(q,p) = mean(cell2mat(MLC.table.individuals(ind).cost(:,1)));
%               for k=1:NJcomponents
%                   Jcomponents(k,q,p) = mean(cell2mat(MLC.table.individuals(ind).cost(:,k+1)));
%               end
%           elseif HowToPlot==2
%               J(q,p) = max(cell2mat(MLC.table.individuals(ind).cost(:,1)));
%               for k=1:NJcomponents
...
%       end
%   end
%   axis([0 NGen 0.9*min(J(:)) max(J(:))+1e-3])
%   hold off
%   % labels
...
%   % Monte Carlo
%       plot(1:PopSize,J(Jorder(:,1),1),'bo')
%   % Evolution
%   for p=2:NGen
%       for q=1:PopSize
...
%       end
%   end
%   axis([1 PopSize*NGen 0.9*min(J(:)) max(J(:))+1e-3])
%   hold off
%   % labels
...
%     for gen=1:NGen
%         hold on
%         plot(Jb(:,gen),Ja(:,gen),'.','MarkerSize',20,'Color',cmp(gen,:))
%         hold off
%     end
...
%     
%     % axis
%     minJa = min(min(Ja));maxJa = max(max(Ja)); dJa = maxJa-minJa;
%     minJb = min(min(Jb));maxJb = max(max(Jb)); dJb = maxJb-minJb;
...
%     for gen=1:NGen
%         hold on
%         plot(PopSize*(gen-1)+(1:PopSize),J(:,gen),'.','Color',cmp(gen,:),'MarkerSize',12.5)
%         hold off
%     end
%     
%     % labels and ticks
...
%     for gen=1:NGen
%         hold on
%         plot(PopSize*(gen-1)+(1:PopSize),JC(:,gen),'.','Color',cmp(gen,:),'MarkerSize',12.5)
%         hold off
%     end
%     
%     % labels and ticks
...
function [x,y] = convergence(MLC,Pareto,plt)
    ...
    % figure.
    %
    % Guy Y. Cornejo Maceda, 04/26/2020
    %
    % See also spectrogram, convergence
...
subplot(NJcomponents+1,1,1)
    ...
    for gen=1:NGen
        for q=1:PopSize
            if isBad(q,gen)
                plot(PopSize*(gen-1)+(q),maxJ,'r*','MarkerSize',12.5)
            else
                plot(PopSize*(gen-1)+(q),J(q,gen),'.','Color',cmp(gen,:),'MarkerSize',12.5)
            end
        end
    end
    hold off
...
for NJC=1:NJcomponents
    ...
    for gen=1:NGen
        for q=1:PopSize
            if isBad(q,gen)
                plot(PopSize*(gen-1)+(q),maxJC,'r*','MarkerSize',12.5)
            else
                plot(PopSize*(gen-1)+(q),JC(q,gen),'.','Color',cmp(gen,:),'MarkerSize',12.5)
            end
        end
    end
    hold off
