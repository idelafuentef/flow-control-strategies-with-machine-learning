...
%% Fill properties
    % Basket
    gMLC.basket = gMLCSource.basket.copy(gMLCSource.parameters);
    
    % Stock
    fprintf('Stock is not processed for now...\n');
    
    % Landscapebasket
    if not(isempty(gMLCSource.landscapebasket))
        LandscapeCycle = find(gMLCSource.landscapebasket(:,1)==MaxCycle);
        gMLC.landscapebasket = gMLCSource.landscapebasket(1:LandscapeCycle,:);
    end
    ...
    if (MaxCycle==-1)
        ...
        gMLC.basket.status.cycle = MaxCycle;
    end
%% Update properties
    % Basket
        % Update labels and cost.
        gMLC.basket.status.last_operation = 'importation';
        new_labels = gMLC.basket.update(gMLC.table,gMLC.parameters);
        gMLC.basket.status.last_operation = 'importation';
    % History
