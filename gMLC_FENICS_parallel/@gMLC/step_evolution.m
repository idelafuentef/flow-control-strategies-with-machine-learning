...
%
% See also step_exploitation, step_exploration.
...
%% Parameters
    ...
    last_ope = gMLC.basket.status.last_operation;
...
PopulationLabels = gMLC.build_evolution_basket; % unless external in this case load
if strcmp(problem_type,'external')
    % Is there individuals to interpolate?
    to_build = gMLC.basket.to_build;
...
ID = zeros(length(IDs),1);
  for p=1:NumberOffsprings
    ...
    % To evaluate
    gMLC.basket.status.individuals_to_evaluate = [gMLC.basket.status.individuals_to_evaluate;ID];
    ...
    % External evaluation test
        if strcmp(problem_type,'external')
          ...
          gMLC.basket.waiting_room.labels = [gMLC.basket.waiting_room.labels;ID];
          if VERBOSE > 3, fprintf('. Ready\n'),end
            % Nothing to continue
            % Update
            gMLC.basket.status.last_operation = 'evolution';
          return
        end
    if VERBOSE > 0, fprintf(':\n'),end
%     % The control law is evaluated here, so no need to put in the individuals_to_evaluate array. (original version)
%     % But if there were to not be evaluated, they need to be added to the array
%         Jev = -1*ones(length(IDs),1);
%         cycle = gMLC.basket.status.cycle+1;
%         for p=1:length(IDs)
%           jev = gMLC.table.evaluate(ID(p),gMLC.parameters,0); % 0=no visu
%           N_ev = length(gMLC.basket.status.individuals_to_evaluate)+p-length(IDs);
%           gMLC.table.individuals(ID(p)).evaluation_order = [cycle,N_ev]; % Previously cycle+0.5
%           Jev(p) = jev;
%             if (VERBOSE > 0) && mod(p,10)==0, fprintf('\n'),end
%         end

    % The control law is evaluated here, so no need to put in the individuals_to_evaluate array. (modified version by Philipp Oswald for parallelization)
    % But if there were to not be evaluated, they need to be added to the array
        Jev = -1*ones(length(IDs),1);
        cycle = gMLC.basket.status.cycle+1;
        ListIndivs(length(IDs)) = gMLCind;
        for p = 1:length(IDs)
            ListIndivs(p) = gMLC.table.individuals(ID(p));
        end
        temp1 = gMLC.parameters;
        parfor p = 1:length(IDs)
            my_ind = ListIndivs(p);
            Jev(p) = my_ind.evaluate(temp1,0);
            ListIndivs(p) = my_ind;
        end
        for p = 1:length(IDs)
            N_ev = length(gMLC.basket.status.individuals_to_evaluate)+p-length(IDs);
            gMLC.table.individuals(ID(p)) = ListIndivs(p);
            gMLC.table.individuals(ID(p)).evaluation_order = [cycle,N_ev]; % Previously cycle+0.5
            % Update properties
            if (~isnan(Jev(p)))&&(~isinf(Jev(p)))
                gMLC.table.evaluated(ID(p))=1;
            else
                gMLC.table.evaluated(ID(p))=NaN;
            end
            if (VERBOSE > 0) && mod(p,10)==0, fprintf('\n'),end
        end
    ...
    %% Update properties
        % Individuals informations
            gMLC.basket.waiting_room.labels = [gMLC.basket.waiting_room.labels;ID];
            gMLC.basket.waiting_room.costs = [gMLC.basket.waiting_room.costs;Jev];
        % Stat
            gMLC.basket.status.last_operation = 'evolution';
...
%------------------------------------------------------------------
            ...
            % Update history
            switch gMLC.basket.status.last_operation
            case 'evolution'
              fact_type = 6;
            case 'reflect'
              fact_type = 11;
