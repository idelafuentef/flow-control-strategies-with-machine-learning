...
%%  Step0 : Initialization
        if strcmp(gMLC.basket.status.last_operation,'NULL')
...
%% Exploration step : Downhill simplex 
    gMLC.new_cycle(1)
    if not(strcmp(last_operation,'numerical DS'))...
            && not(strcmp(last_operation,'DS interpolation'))
    gMLC.step_exploitation;
    end
    if external_interpolation && strcmp(gMLC.basket.status.last_operation,'numerical DS')
        fprintf('Interpolate %i simplex individuals!\n',size(gMLC.basket.to_build,1))
        return
    end
%% Exploitation step : Latin Hypercube Simpling (LHS)
    gMLC.step_exploration;
