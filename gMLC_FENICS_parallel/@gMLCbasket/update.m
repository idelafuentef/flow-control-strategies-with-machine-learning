function [new_labels,gMLC_basket]=update(gMLC_basket,gMLC_table,gMLC_parameters)
% gMLCbasket class update method
% Updates the basket with the individuals (labels and costs) in the
% waiting_room.
%
...
if VERBOSE > 6, fprintf('     Update basket - Start\n'),end

%% Labels
    labels = gMLC_basket.labels;
    costs = gMLC_basket.costs;
    waiting_labels = gMLC_basket.waiting_room.labels;
    waiting_costs = gMLC_basket.waiting_room.costs;

%% External
    if strcmp(problem_type,'external')
      gMLC_basket.status.last_operation = 'external';
    end

    %% Switch
    last_operation = gMLC_basket.status.last_operation;
...
%                 end
        ...
        case {'Random addition','LHS addition'}
            wlabel= waiting_labels;
            wcost= waiting_costs;
            STCP = true;
            for p=1:length(wlabel)
                STCP = STCP && sum(strcmp(gMLC_table.individuals(wlabel(p)).description.type,{'Random','LHS'}));
            end
            if STCP
                ...
                error('Error type in the waiting room - LHS\n')
            end

        case {'no exploration','no exploitation'}
            new_labels = labels;
            new_costs = costs;
        case 'external'
            new_labels = NaN;
        ...
        case 'importation'
            if gMLC_table.number<BS
                error('Not enough individuals, lower basket size')
            end
...
%% Update properties
    % Individuals informations
        if length(new_labels)==BS% TEST NUMBER OF ELEMENTS
            gMLC_basket.labels = new_labels;
            gMLC_basket.costs = new_costs;
        else
            fprintf('Error number of elements in the new individuals\n')
            return
        end

        gMLC_basket.waiting_room.labels = []; % structure : labels, costs
        gMLC_basket.waiting_room.costs = []; % structure : labels, costs
    % Centroid
%         gMLC_basket.centroid = NaN*gMLC_basket.centroid;
    % Stat
        gMLC_basket.status.evaluated = 'updated';
        gMLC_basket.status.individuals_to_evaluate = [];


if VERBOSE > 6, fprintf('     Update basket - End\n'),end
