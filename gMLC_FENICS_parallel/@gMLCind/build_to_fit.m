...
%% Parameters
    ...
    ratio_import = gMLC_parameters.ratio_import;
...
%% Initialize MLC
    ...
    % Importation
      % number of control laws imported
        NbImport = round(lgpc.parameters.PopulationSize*ratio_import);
      % labels
          import_labels = gMLC_table.best_individuals(NbImport,0); % 0 : matrix, 1 : evaluated
      % import
        if NbImport>0,BoxIndividuals(NbImport) = MLCind;end
        for p=1:NbImport
          Indp = gMLC_table.individuals(import_labels(p));
          Indi = Indp.gMLC2MLC(lgpc.parameters,gMLC_table.ControlPoints(import_labels(p),:));
          BoxIndividuals(p)=Indi;
        end
        if NbImport>0,lgpc.import(BoxIndividuals);end
