...
%% Parallel evaluation loop (edited by Philipp)
% % Modification-----------------------------------------------------------------
% ListIndivs(numel(idx_to_evaluate)) = MLCind;
% for p=1:numel(idx_to_evaluate)
%     ListIndivs(p) = MLC_table.individuals(indiv_to_evaluate(p));
% end
% temp1 = MLC_parameters.verbose;
% temp2 = MLC_parameters.PopulationSize;
% parfor p=1:numel(idx_to_evaluate)
%     if temp1
%         fprintf('    Evaluation of individual %i/%i',idx_to_evaluate(p),temp2)
%     end
%     my_ind = ListIndivs(p);
%     my_ind.evaluate_indiv(MLC_parameters);
%     ListIndivs(p) = my_ind;
% end
% for p=1:numel(idx_to_evaluate)
%     MLC_table.individuals(indiv_to_evaluate(p)) = ListIndivs(p);
% end
% Modification-----------------------------------------------------------------
