function  J_out = toy_problem(b_cell,parameters,ID,direc,visu)
% try
%% Objective
...
%% Equation resolution
    ...
    b_val = b(time);
    J_out = {trapz(time,(tanh_val-b_val).^2)};
...
if (nargin > 2) && ID
  ...
  % Actuation
	Actuations = b_val;
  Actuations = [Times,Actuations'];
  save([direc,'/Actuations/ID',num2str(ID),'.dat'],'Actuations','-ascii')
end
