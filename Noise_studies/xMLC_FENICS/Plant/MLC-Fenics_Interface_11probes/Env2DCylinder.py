...
# ------------------------------------------------------------------------
    ...
    def execute(self,fun_jet1,fun_jet2,dt):
        ...
        for crrt_probe in range(len(self.output_params["locations"])):
            ...
            rand_add=np.random.randint(50)*0.001
            Probe_u.append(self.probes_values[2 * crrt_probe]                                           +rand_add)
            rand_add=np.random.randint(5)*0.001
            Probe_v.append(self.probes_values[2 * crrt_probe + 1]                                       +rand_add)
            
            rand_add=np.random.randint(5)*0.001
            Probe_u_1_2.append(self.history_parameters["probe_{}_u".format(crrt_probe)].get()[-334]     +rand_add)
            rand_add=np.random.randint(5)*0.001
            Probe_v_1_2.append(self.history_parameters["probe_{}_v".format(crrt_probe)].get()[-334]     +rand_add)
            
            rand_add=np.random.randint(5)*0.001
            Probe_u_1_4.append(self.history_parameters["probe_{}_u".format(crrt_probe)].get()[-167]     +rand_add)
            rand_add=np.random.randint(5)*0.001
            Probe_v_1_4.append(self.history_parameters["probe_{}_v".format(crrt_probe)].get()[-167]     +rand_add)
            
            rand_add=np.random.randint(5)*0.001
            Probe_u_3_4.append(self.history_parameters["probe_{}_u".format(crrt_probe)].get()[-501]     +rand_add)
            rand_add=np.random.randint(5)*0.001
            Probe_v_3_4.append(self.history_parameters["probe_{}_v".format(crrt_probe)].get()[-501]     +rand_add)
        
        #actions are defined as the evaluation of the (string) function assigned to the jets
        #these functions are assigned previously in "perform_learning.py" 
